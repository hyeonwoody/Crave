...
static size_t _CurlWriteStepCallback(void* contents, size_t size, size_t nmemb, void* output) {
    size_t totalSize = size * nmemb;
    CNamuStep::CurlBuffer *data = (CNamuStep::CurlBuffer *)output;
    data->privateData = (uint8_t *) realloc (data->privateData, data->transferedSize + totalSize + 1); 
    if (data->privateData == NULL)
    {
        return 0;
    }
    memcpy(data->privateData + data->transferedSize, contents, totalSize);
    data->transferedSize += totalSize;
    ((uint8_t*)data->privateData)[data->transferedSize] = 0;
    return totalSize;
}
